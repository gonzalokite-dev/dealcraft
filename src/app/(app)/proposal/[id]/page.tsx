...
import { CANONICAL_ORDER, SECTION_LABELS, PROPOSAL_TYPE_LABELS, PDF_TEMPLATES } from "@/lib/proposals/constants";
    ...
    try {
      const res = await fetch("/api/proposals/section", {
        ...
        body: JSON.stringify({
          ...
          current_content: content[sectionKey],
          action,
        }),
      });
      const data = await res.json();
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-border bg-surface sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0">
            ...
            {statusInfo && (
              <span className={`text-xs font-medium px-2.5 py-1 rounded-full flex-shrink-0 hidden sm:inline ${statusInfo.class}`}>
                {statusInfo.label}
              </span>
            )}
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        ...
        {activeSections.map((key) => (
          <div key={key} className="bg-surface border border-border rounded-2xl p-6 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-secondary uppercase tracking-widest block" htmlFor={key}>
              ...
              <div className="relative" onClick={(e) => e.stopPropagation()}>
                ...
                {openMenu === key && (
                  <div className="absolute right-0 top-8 z-20 bg-surface border border-border rounded-xl shadow-lg py-1 min-w-[140px]">
            ...
            <textarea
              ...
              disabled={sectionLoading === key}
              className="w-full text-sm text-secondary leading-relaxed bg-background border border-border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors resize-none disabled:opacity-50"
            />
          </div>
        ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {PDF_TEMPLATES.map((t) => (
              <button
                key={t.value}
                type="button"
                onClick={() => setTemplate(t.value as PDFTemplateName)}
        ...
        {/* Bottom actions */}
        <div className="flex justify-between gap-3 pt-2 pb-10 flex-wrap">
          <button
            onClick={handleShare}
            disabled={sharing}
            className="text-sm font-medium border border-border text-secondary px-6 py-2.5 rounded-full hover:border-gray-400 transition-colors disabled:opacity-50"
          >
          ...
          <div className="bg-surface rounded-2xl border border-border shadow-2xl w-full max-w-md p-8 space-y-5">
            <div>
              ...
              <p className="text-sm text-muted mt-1">
                Comparte este enlace con tu cliente. Recibirás una notificación cuando la vea o la apruebe.
              </p>
            </div>
            ...
            <div className="flex gap-2 text-xs text-muted bg-background border border-border rounded-lg px-4 py-3">
              ...
              <span>El estado de la propuesta se actualiza automáticamente cuando tu cliente la abre o la aprueba.</span>
            </div>
